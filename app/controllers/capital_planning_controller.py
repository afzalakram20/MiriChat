...
class CapitalPlanningController:
    ...
    async def estimate_materials(self, req: ScopeRequest):
        try:
            ...
            # Pull dynamic model from incoming payload (req is dict-like in our routes)
            model_key = str(req.get("model_key", "")).strip()  # type: ignore[attr-defined]
            model_id = str(req.get("model_id", "")).strip()    # type: ignore[attr-defined]
            if not model_key:
                raise HTTPException(status_code=400, detail="Model key is required")
            if not model_id:
                raise HTTPException(status_code=400, detail="Model is required")

            material_result: MaterialExtractionResult = await service.llm_extract_materials(
                req, model_key=model_key, model_id=model_id
            )
    ...
    def analyze_project_command(self, user_command: str, model_key: str | None = None, model_id: str | None = None) -> ProjectAutoGenerated:
        # 1) Call LLM chain → strongly typed intent
        log.info("user_command: %s", user_command)
        intent: ProjectIntentLLM = create_project_intent_chain(model_key=model_key, model_id=model_id).invoke(
