...
log = logging.getLogger("app.services.best_selection")
client = OpenAI(api_key=settings.OPENAI_API_KEY)


def build_best_selection_payload(raw_data: dict) -> dict:
    """
    raw_data is your whole JSON (with 'ok', 'data', 'error').
    tasks is a list of human-readable category names in the same order as data[].
    """
    categories = raw_data["data"]  # this is already a list of {category, query, items}

    # Optionally inject task names into category.category if empty
    for i, cat in enumerate(categories):
        if not cat.get("category"):
            if i < len(tasks):
                cat["category"] = tasks[i]
            else:
                cat["category"] = cat.get("query", f"category_{i+1}")
...
def call_best_selection_llm(raw_data: dict):
    payload = raw_data

    completion = client.chat.completions.create(
        model="gpt-5.1",
        messages=[
            {"role": "system", "content": BEST_PRICE_SELECTION_SYSTEM_PROMPT},
            {"role": "user", "content": json.dumps(payload)},
        ],
        temperature=0,
        response_format={"type": "json_object"},  # we expect pure JSON
    )

    content = completion.choices[0].message.content
    log.info("selected content: %s", content)
